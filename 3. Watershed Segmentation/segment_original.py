...
def process_image(image_path, binary_path, output_image_path):
    ...
    # ** NEW PART: Draw borders instead of random colors **
    output_image = image.copy()
    num_segments = np.max(updated_markers) + 1

    # Loop through each region and draw its border
    for label in range(1, num_segments):
        region_mask = (updated_markers == label).astype(np.uint8)
        
        # Extract contours of the region
        contours, _ = cv2.findContours(region_mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
        
        for contour in contours:
            cv2.drawContours(output_image, [contour], -1, (0, 0, 0), thickness=1)  # Black border, 2px thickness
